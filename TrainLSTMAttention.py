...
from modules import Model, LSTM_Attention_Model
...
def main(opt):
    ...
    forecast_model = LSTM_Attention_Model(input_size = opt.config.num_parameters,
                                output_size = 1)
    ...
    for epoch in tqdm(range(opt.config.num_epoch)):
        ...
        if val_loss <= best_score:
            torch.save(
                ...
                f"checkpoint/{opt.name}_forecastAttention_model.pt"
            )
            best_score = val_loss
    ...
    forecast_model.load_state_dict(torch.load(f"checkpoint/{opt.name}_forecastAttention_model.pt"))
    ...
    evaluate_pred.to_csv(f"{opt.name}_LSTMAttention_evaluate_result.csv")
    ...
    plt.title(f"LSTM-Attention prediction test dataset with MSE: {mse}, RMSE: {rmse}")
    plt.ylabel(f"{opt.name}")
    plt.xlabel("Day")
    plt.legend(loc="upper right")
    plt.savefig(f'{opt.name}_LSTMAttention_evaluate_fig.png')


    with open(f'checkpoint/{opt.name}_LSTMAttention_xscaler.pkl', 'wb') as fopen:
        pickle.dump(x_scaler, fopen)
    with open(f'checkpoint/{opt.name}_LSTMAttention_yscaler.pkl', 'wb') as fopen:
